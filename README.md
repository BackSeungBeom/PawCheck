...
`prisma/dev.db`에 한국관광공사 실 API로 이미 동기화한 데이터(관광지+레포츠 937개 시설,
LLM 예외조항 파싱 완료분 포함)가 **저장소에 그대로 포함되어 있어** 별도 동기화 없이 바로 사용
가능합니다. 스키마 변경이 없으면 아래 명령은 생략해도 됩니다.
...
이미 채워진 데이터를 최신화하거나 직접 API 연동을 검증하고 싶다면 실행하세요.
(자체 서비스키 필요 — 없으면 fixture 데이터로 mock 실행됩니다.)
